...
import { issueLikes, issues } from '../db/schema.js'
import { IssueInteractionsResponseSchema } from './schemas/issue-interactions.js'
...
export const getIssueInteractions = new OpenAPIHono<{
  ...
  if (user) {
    const likes = await db
      ...
      .from(issueLikes)
      .where(
        and(eq(issueLikes.userId, user.id), inArray(issueLikes.issueId, issueIdArray))
      )
